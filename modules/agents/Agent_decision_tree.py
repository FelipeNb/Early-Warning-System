...
# import procname
# procname.setprocname(str(sys.argv[1])+'Agent_decision_tree')

ipAddress = '127.0.0.1'
port = int(sys.argv[1])

urls = (
    '/Agent_decision_tree', 'Main',
)
...
i = time.time()
classifier, accuracy = func_Decision_Tree(
    x_train, y_train, x_test, y_test, plot=False)
f = time.time()
# print('Decision_Tree' + str(f-i))
############  INIT VARIABLES  ##############
...
class Main:
    def POST(self):
        ...
        if param:
            ...
            obj["error"] = [
                {"message": "Sorry, bad request, params missing (HAVE TO BE " + n_param + "PARAMETERS)", "code": 400}]
        gc.collect()
