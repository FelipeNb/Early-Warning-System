...
#import procname
#procname.setprocname(str(sys.argv[1])+'Agent_knn')
...
# print_importance_features(df,x_train, y_train)

n_param = len(x_train[0])
i = time.time()
classifier, accuracy = func_Knn(x_train,y_train,x_test,y_test,plot=False)
f = time.time()
# print("Knn" + str(f-i))
############  INIT VARIABLES  ##############
